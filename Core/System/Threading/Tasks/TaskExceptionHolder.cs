    ...
    internal class TaskExceptionHolder
    {
        ...
        ~TaskExceptionHolder()
        {
                ...
                !Environment.HasShutdownStarted && !AppDomain.CurrentDomain.IsFinalizingForUnload() && !s_domainUnloadStarted)
            {
                ...
                AggregateException exceptionToThrow = new AggregateException("A Task's exception(s) were not observed either by Waiting on the Task or accessing its Exception property. As a result, the unobserved exception was rethrown by the finalizer thread.", (IEnumerable<Exception>)m_faultExceptions);
