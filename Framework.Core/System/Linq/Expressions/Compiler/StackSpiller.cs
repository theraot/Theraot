    ...
    internal sealed partial class StackSpiller
    {
        ...
        internal Expression<T> Rewrite<T>(Expression<T> lambda)
        {
            ...
            if (body.Action != RewriteAction.None)
            {
                ...
                {
                    newBody = Expression.Block(_tm.Temps, ArrayReadOnlyCollection<Expression>.Create(newBody));
                }
        ...
        private Result RewriteListInitExpression(Expression expr, Stack stack)
        {
            ...
            switch (action)
            {
                ...
                case RewriteAction.Copy:
                    ...
                    expr = new ListInitExpression((NewExpression)rewrittenNew, ArrayReadOnlyCollection<ElementInit>.Create(newInitializer));
                    break;
        ...
        private Result RewriteNewArrayExpression(Expression expr, Stack stack)
        {
            ...
            if (cr.Rewrite)
            {
                expr = NewArrayExpression.Make(node.NodeType, node.Type, ArrayReadOnlyCollection<Expression>.Create(cr[0, -1]));
            }
